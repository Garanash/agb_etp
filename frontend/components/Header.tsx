...
import { Menu, X, Building2, User, LogIn, LogOut } from 'lucide-react'
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // Можно загрузить имя пользователя из токена или сделать запрос
      fetchUserInfo()
    }
  }, [])
          ...
          <div className="hidden md:flex items-center space-x-4">
            {isAuthenticated ? (
              <>
                <Link 
                  href="/dashboard" 
                  className="text-secondary-700 hover:text-primary-600 px-3 py-2 text-sm font-medium transition-colors flex items-center space-x-1"
                >
                ...
                <button
                  ...
                  className="text-secondary-700 hover:text-red-600 px-3 py-2 text-sm font-medium transition-colors flex items-center space-x-1"
                >
