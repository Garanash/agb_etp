  ...
  const getNavigationItems = () => {
    ...
    switch (userRole) {
      ...
      case 'contract_manager':
        return [
          ...baseItems,
          { href: '/dashboard/tenders', label: 'Мои тендеры', icon: FileText },
          { href: '/dashboard/tenders/create', label: 'Создать тендер', icon: Plus },
          { href: '/dashboard/applications', label: 'Заявки', icon: Eye },
        ]
      case 'manager':
        return [
          ...baseItems,
          { href: '/dashboard/tenders', label: 'Тендеры', icon: FileText },
          { href: '/dashboard/suppliers', label: 'Поставщики', icon: Users },
        ]
      case 'supplier':
        return [
          ...baseItems,
          { href: '/dashboard/tenders', label: 'Доступные тендеры', icon: FileText },
          { href: '/dashboard/applications', label: 'Мои заявки', icon: FileText },
        ]
      default:
        return baseItems
    }
  ...
  return (
    <div className="min-h-screen bg-secondary-50">
      ...
      <div className="md:hidden">
        <div className="bg-white shadow-sm border-b border-secondary-200 px-4 py-3">
            ...
            <nav className="px-4 py-2 space-y-1">
              {navigationItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="flex items-center space-x-3 px-3 py-2 text-sm text-secondary-700 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors"
      ...
      <div className="hidden md:flex">
        {/* Боковая панель */}
        <div className="w-64 bg-white shadow-sm border-r border-secondary-200 min-h-screen">
          <div className="p-6 border-b border-secondary-200">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-lg font-bold text-secondary-900">Алмазгеобур</span>
            </div>
            <div className="text-sm text-secondary-600">
              {userName} ({getRoleLabel(userRole)})
            </div>
          ...
          <div className="absolute bottom-4 left-4 right-4">
            <button
              onClick={handleLogout}
              className="w-full flex items-center space-x-3 px-3 py-2 text-sm text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
            >
              <LogOut className="h-5 w-5" />
              <span>Выйти</span>
            </button>
